...
export const bankOfAmericaAdapter: BankAdapterConfig = {
  ...
  extractors: {
    accounts: {
      ...
      fields: [
        { fieldName: 'accountName', selector: '.AccountName, .account-name-mask', strategy: { type: 'textContent' }, transform: 'trim', required: true },
        { fieldName: 'accountNumber', selector: '.AccountNumber, .account-number', strategy: { type: 'textContent' }, transform: 'trim', required: false },
        { fieldName: 'balance', selector: '.AccountBalance, .balance-value', strategy: { type: 'textContent' }, transform: 'parseAmount', required: true },
        { fieldName: 'accountType', selector: '.account-type-label', strategy: { type: 'textContent' }, transform: 'lowercase', required: false },
      ],
    },
    transactions: {
      ...
      fields: [
        { fieldName: 'date', selector: '.trans-date-col, td.date-col', strategy: { type: 'textContent' }, transform: 'parseDate', required: true },
        { fieldName: 'description', selector: '.trans-desc-col, td.desc-col', strategy: { type: 'textContent' }, transform: 'trim', required: true },
        { fieldName: 'amount', selector: '.trans-amount-col, td.amount-col', strategy: { type: 'textContent' }, transform: 'parseAmount', required: true },
        { fieldName: 'status', selector: '.trans-status, .pending-indicator', strategy: { type: 'textContent' }, transform: 'lowercase', required: false },
        { fieldName: 'category', selector: '.trans-category', strategy: { type: 'textContent' }, transform: 'trim', required: false },
      ],
    },
  },
  ...
  mfaDetector: {
    ...
    rules: [
      { selector: '#VerifyOTPForm, .sms-challenge', challengeType: 'sms_code', contextSelector: '.masked-phone, .phone-hint', priority: 10 },
      { selector: '.email-otp-challenge, [data-testid="email-challenge"]', challengeType: 'email_code', contextSelector: '.masked-email, .email-hint', priority: 20 },
      { selector: '#challenge-question, .security-question-form', challengeType: 'security_questions', contextSelector: '.question-text, #challenge-question', priority: 30 },
      { selector: '.push-auth-prompt, .device-verification', challengeType: 'push_notification', contextSelector: '.device-info', priority: 40 },
    ],
  },
} as const;
