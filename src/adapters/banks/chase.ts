...
export const chaseAdapter: BankAdapterConfig = {
  ...
  extractors: {
      ...
      fields: [
        {
          ...
          selector: '.account-name, .tile-accountName',
        ...
        },
        {
          ...
          selector: '.account-number, .mask-number',
          strategy: { type: 'textContent' },
          transform: 'trim',
          required: false,
        },
        {
          fieldName: 'balance',
          selector: '.account-balance, .tile-amount',
          strategy: { type: 'textContent' },
    ...
    transactions: {
      ...
      fields: [
        {
          fieldName: 'date',
          selector: '.transaction-date, .trans-date',
          strategy: { type: 'textContent' },
          transform: 'parseDate',
          required: true,
        },
        {
          ...
          selector: '.transaction-description, .trans-desc',
          strategy: { type: 'textContent' },
          transform: 'trim',
          required: true,
        },
        {
          fieldName: 'amount',
          selector: '.transaction-amount, .trans-amount',
          strategy: { type: 'textContent' },
          transform: 'parseAmount',
          required: true,
        },
        {
          fieldName: 'status',
          selector: '.transaction-status, .pending-label',
          strategy: { type: 'textContent' },
          transform: 'lowercase',
          required: false,
        },
  ...
  mfaDetector: {
    ...
    rules: [
      {
        selector: '#otpcode_input-input-field',
        challengeType: 'sms_code',
        contextSelector: '.phone-mask, .masked-phone',
        priority: 10,
      },
      {
        selector: '.email-verification, [data-testid="email-otp"]',
        challengeType: 'email_code',
        contextSelector: '.email-mask, .masked-email',
        priority: 20,
      },
      {
        selector: '.security-question, [data-testid="security-question"]',
        challengeType: 'security_questions',
        contextSelector: '.question-text',
        priority: 30,
      },
      {
        selector: '.push-notification-prompt, [data-testid="push-verify"]',
        challengeType: 'push_notification',
        contextSelector: '.device-name',
        priority: 40,
      },
    ],
  },
