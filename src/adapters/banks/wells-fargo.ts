...
export const wellsFargoAdapter: BankAdapterConfig = {
      ...
      fields: [
        {
          ...
          selector: '.account-name, .acct-name',
        ...
        },
        {
          ...
          selector: '.account-number, .acct-number',
          strategy: { type: 'textContent' },
          transform: 'trim',
          required: false,
        },
        {
          fieldName: 'balance',
          selector: '.account-balance, .available-balance',
          strategy: { type: 'textContent' },
    ...
    transactions: {
      ...
      fields: [
        ...
        },
        {
          ...
          selector: '.trans-description, td.description',
          strategy: { type: 'textContent' },
          transform: 'trim',
          required: true,
        },
        {
          fieldName: 'amount',
          selector: '.trans-amount, td.amount',
          strategy: { type: 'textContent' },
          transform: 'parseAmount',
          required: true,
        },
        {
          fieldName: 'status',
          selector: '.trans-status, .pending-text',
          strategy: { type: 'textContent' },
          transform: 'lowercase',
          required: false,
        },
      ...
      fields: [
        {
          ...
          selector: '.full-account-number, #fullAcctNumber',
          strategy: { type: 'textContent' },
        ...
        },
        {
          ...
          selector: '.routing-number, #routingNumber',
          strategy: { type: 'textContent' },
          transform: 'stripWhitespace',
          required: true,
        },
  ...
  mfaDetector: {
    ...
    rules: [
      {
        selector: '#otp, input[name="otp"]',
        challengeType: 'sms_code',
        contextSelector: '.phone-display, .masked-phone',
        priority: 10,
      },
      {
        selector: '.email-verification-form, #emailOTP',
        challengeType: 'email_code',
        contextSelector: '.email-display, .masked-email',
        priority: 20,
      },
      {
        selector: '#challengeQuestion, .challenge-question',
        challengeType: 'security_questions',
        contextSelector: '.challenge-question, #challengeQuestion',
        priority: 30,
      },
      {
        selector: '.push-notification, #pushAuthPrompt',
        challengeType: 'push_notification',
        contextSelector: '.device-display',
        priority: 40,
      },
    ],
  },
